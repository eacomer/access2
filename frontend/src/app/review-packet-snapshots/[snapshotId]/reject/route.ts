...
type RejectSnapshotRequestBody = {
  reason?: unknown;
};
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  let body: RejectSnapshotRequestBody;
  try {
    body = (await request.json()) as RejectSnapshotRequestBody;
  } catch {
    return NextResponse.json({ error: "Invalid rejection request JSON." }, { status: 400 });
  }
  ...
  if (!authToken) {
    return NextResponse.json({ error: "Sign in is required to reject a review packet snapshot." }, { status: 401 });
  }
  ...
  if (!upstreamResponse.ok) {
    return NextResponse.json(
      {
        error: toErrorMessage(payload, "Review packet snapshot rejection failed."),
      },
