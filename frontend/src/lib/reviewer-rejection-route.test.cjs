...
const routePath = path.join(
  __dirname,
  "../app/review-packet-snapshots/[snapshotId]/reject/route.ts",
);
...
test("reviewer rejection route validates missing and blank reasons without calling backend", async () => {
  ...
  assert.deepEqual(await missing.json(), { error: "Rejection reason required." });

  const blank = await callRoute({ reason: "   " });
  assert.equal(blank.status, 400);
  assert.deepEqual(await blank.json(), { error: "Rejection reason required." });
});

test("reviewer rejection route forwards valid rejection payload with decision note", async () => {
  const calls = [];
  const payload = {
    id: "snapshot-1",
    review_status: "rejected",
    decision_note: "Outcome evidence is incomplete.",
  };
  ...
  const response = await callRoute({ reason: "  Outcome evidence is incomplete.  " });
...
test("reviewer rejection route preserves backend 422 and 409 errors", async () => {
  const cases = [
    { status: 422, detail: "Rejection reason is required." },
  ...
  for (const entry of cases) {
    ...
    const response = await callRoute({ reason: "Not enough evidence." });
...
test("reviewer rejection route requires auth before calling backend", async () => {
  authToken = null;
  globalThis.fetch = async () => {
  ...
  assert.deepEqual(await response.json(), {
    error: "Sign in is required to reject a review packet snapshot.",
  });
});
